    ...
    if form.is_valid() and series_form.is_valid():
        ...
        ensure_service_serials(
            ...
            serials_map={
                ...
                ('M', 'T'): data.get('serial_mudanza') or '',
                ('D', 'T'): data.get('serial_delivery') or '',
                ('C', 'T'): data.get('serial_carga') or '',
...
def save_subsidiary_edit(request, subsidiary_id: int, company_id: int):
    ...
    serial_fields = {
        ...
        ('M', 'T'): _val('serial_mudanza'),
        ('D', 'T'): _val('serial_delivery'),
        ('C', 'T'): _val('serial_carga'),
    ...
    too_long = [
        label for label, value in (
            ...
            ('Serie mudanzas', serial_fields[('M', 'T')]),
            ('Serie delivery', serial_fields[('D', 'T')]),
            ('Serie carga', serial_fields[('C', 'T')]),
            ('Serie manifiestos', serial_fields[('P', 'T')]),
        )
        if len(value) > 10
    ]
    if too_long:
