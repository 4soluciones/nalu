...
from .models import Owner, Programming, Truck, TruckBrand, TruckModel
...
def new_guide(request):
    ...
    service_type = (request.GET.get('service_type') or 'E').upper()
    if service_type not in ('E', 'M', 'D', 'C'):
        service_type = 'E'
    service_docs = {}
    for st in ('M', 'D', 'C'):
        service_docs[st] = {
            'serial': get_serial_service(subsidiary_obj, company_rotation_obj, st, 'T') or '',
            'correlative': get_correlative_service(subsidiary_obj, company_rotation_obj, st, 'T') or '',
        }
    service_docs['E'] = {}
...
def create_order(request):
    if request.method == 'GET':
        ...
        service_type = str(data_orders.get('Service_Type', 'E'))
        ...
        if service_type in ('M', 'D', 'C'):
            save_service_for_order(order_obj, service_type, service_extra)
