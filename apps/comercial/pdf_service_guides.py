...
from apps.sales.models import Order, OrderAction, PROPERTY_TYPE_CHOICES, PAYMENT_METHOD_CHOICES
...
def _company_block(order_obj):
    ...
    return f'{order_obj.company.business_name}\nRUC: {order_obj.company.ruc}'


def _property_label(code):
    return dict(PROPERTY_TYPE_CHOICES).get(code or '', code or '-')
...
def _header_block(order_obj, title):
    s = _local_styles()
    elements = [Spacer(-20, -20), Paragraph('NALU', s['enterprise']),
                Paragraph(_company_block(order_obj).replace('\n', '<br />'), s['center']), Spacer(2, 2),
                Paragraph(title, s['docname']),
                Paragraph(f'SERIE: {order_obj.serial} - {order_obj.correlative_sale}', s['docno']), Spacer(5, 5),
                _separator(), Spacer(5, 5), _meta_two_cols(order_obj, THERMAL_WT)]
    return elements


def _header_service_block(order_obj, title):
    """Encabezado para mudanza, delivery y carga: marca grande con espacio superior."""
    _ensure_brand_font()
    s = _local_styles()
    short_name = (order_obj.company.short_name or 'NALU').upper()
    elements = [
        Spacer(1, 14),
        Paragraph(short_name, s['brand_title']),
        Spacer(2, 4),
        Paragraph(_company_block(order_obj).replace('\n', '<br />'), s['center']),
        Spacer(2, 2),
        Paragraph(title, s['docname']),
...
def _finish_ticket(elements, order_obj, pk, service_type, total_font_size=8, request=None):
    ...
    elements.extend([
        ...
        *_footer_terms_flowables(service_type),
        Spacer(1, 1),
        _separator(),
        Paragraph('¡Gracias por confiar en NALU!', s['center_small']),
    ])
...
def build_ticket_for_service(order_obj, pk, request=None):
    builders = {
        'E': build_ticket_encomienda,
        'M': build_ticket_mudanza,
        'D': build_ticket_delivery,
        'C': build_ticket_carga,
    }
    builder = builders.get(order_obj.service_type, build_ticket_encomienda)
    return builder(order_obj, pk, request)
...
def build_bill_encomienda(order_obj, pk):
    ...
    elements.extend([
        ...
        *_footer_terms_flowables('E'),
        Spacer(1, 1),
        _separator(BILL_WT),
        Paragraph('¡Gracias por enviar con NALU!', s['center_small']),
    ])
