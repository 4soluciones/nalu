...
var GuideServices = (function ($) {
    ...
    function setService(svc) {
        ...
        if ($('.nalu-guide-tab').length) {
            $('.nalu-guide-tab').removeClass('active').attr('aria-selected', 'false');
            $('.nalu-guide-tab[data-service="' + svc + '"]').addClass('active').attr('aria-selected', 'true');
        }
        if ($('.nalu-guide-panel').length) {
            $('.nalu-guide-panel').removeClass('is-active');
    ...
    function bindTabs() {
        if (!$('.nalu-guide-tab').length) return;
        $('.nalu-guide-tab').on('click', function () {
    ...
    function applyDocumentNumbers(serial, correlative) {
        $('#id_serie, #id_correlative')
            .val('')
            .prop('readonly', true)
            .addClass('nalu-guide-doc-locked');
        $('#id_serie').val(serial || '');
    ...
    function bindEncomiendas() {
        ...
        $(document).on('click', '.e-remove-detail', function () {
            $(this).closest('.nalu-guide-e-detail-row').remove();
            recalcEncomiendaTotals();
        });
        ...
        $('#e_add_addressee').on('click', function () {
            ...
            $clone.find('.nalu-form-group-action').html(
                '<label>&nbsp;</label><button type="button" class="nalu-btn nalu-btn-outline nalu-btn-sm e-remove-addressee"><i class="fas fa-minus"></i></button>'
            );
    ...
    function clearEncomiendaForm() {
        $('#e_type_bill').val('T').trigger('change');
        $('#e_way_to_pay').val($('#e_way_to_pay option:first').val());
        $('#e_code').val('');
    ...
    function setDocSearchLoading(ui, loading) {
        ...
        $group.find('.nalu-doc-search-overlay').attr('aria-busy', loading ? 'true' : 'false');
        $group.find('.nalu-doc-search-btn').prop('disabled', loading);
    }
    ...
    function addEncomiendaDetail() {
        ...
        $('#e_details_body').append(
            '<div class="nalu-guide-e-detail-row">' +
                ...
                '<button type="button" class="nalu-btn nalu-btn-outline nalu-btn-sm e-remove-detail" title="Quitar">' +
    ...
    function recalcEncomiendaTotals() {
        var sum = 0;
        var count = $('#e_details_body .nalu-guide-e-detail-row').length;
    ...
    function updateServiceDetailState($list, $empty, $countEl) {
        var count = $list.children('.nalu-guide-e-detail-row').length;
    ...
    function bindMudanza() {
        ...
        $('#m_add_item').on('click', function () {
            ...
            $('#m_inventory_body').append(
                '<div class="nalu-guide-e-detail-row nalu-guide-e-detail-row--2">' +
                    '<span class="e-item-qty item-qty">' + q + '</span>' +
                    '<span class="e-item-desc item-desc">' + d.toUpperCase() + '</span>' +
                    '<button type="button" class="nalu-btn nalu-btn-outline nalu-btn-sm m-remove-row e-remove-detail" title="Quitar">' +
                        '<i class="fas fa-trash-alt"></i>' +
                    '</button>' +
                '</div>'
            );
    ...
    function bindDelivery() {
        $('#d_add_item').on('click', function () {
            ...
            $('#d_shipment_body').append(
                '<div class="nalu-guide-e-detail-row nalu-guide-e-detail-row--3">' +
                    '<span class="e-item-desc item-desc">' + d.toUpperCase() + '</span>' +
                    '<span class="e-item-amount item-price text-right">' + p.toFixed(2) + '</span>' +
                    '<button type="button" class="nalu-btn nalu-btn-outline nalu-btn-sm d-remove-row e-remove-detail" title="Quitar">' +
                        '<i class="fas fa-trash-alt"></i>' +
                    '</button>' +
                '</div>'
            );
    ...
    function bindCarga() {
        ...
        $('#c_add_cargo').on('click', function () {
            ...
            if (q <= 0) { toastr.warning('Cantidad inválida'); return; }
            $('#c_cargo_body').append(
                '<div class="nalu-guide-e-detail-row nalu-guide-e-detail-row--cargo">' +
                    '<span class="e-item-qty item-qty">' + q + '</span>' +
                    '<span class="e-item-desc item-desc">' + t.toUpperCase() + '</span>' +
                    '<span class="e-item-amount item-price text-right">' + p.toFixed(2) + '</span>' +
                    '<button type="button" class="nalu-btn nalu-btn-outline nalu-btn-sm c-remove-row e-remove-detail" title="Quitar">' +
                        '<i class="fas fa-trash-alt"></i>' +
                    '</button>' +
                '</div>'
            );
    ...
    function validate() {
        ...
        if (svc === 'E') {
            ...
            if ($('#e_details_body .nalu-guide-e-detail-row').length === 0) { toastr.warning('Agregue detalle de encomienda'); return false; }
            var phoneOk = false;
        ...
        if (svc === 'M') {
            ...
            if ($('#m_inventory_body .nalu-guide-e-detail-row').length === 0) { toastr.warning('Agregue inventario'); return false; }
        ...
        if (svc === 'D') {
            ...
            if ($('#d_shipment_body .nalu-guide-e-detail-row').length === 0) { toastr.warning('Agregue detalle del envío'); return false; }
        ...
        if (svc === 'C') {
            ...
            if ($('#c_cargo_body .nalu-guide-e-detail-row').length === 0) { toastr.warning('Agregue mercancía'); return false; }
        }
        return true;
    }
    ...
    function collectPayload() {
        ...
        if (svc === 'E') {
            ...
            $('#e_details_body .nalu-guide-e-detail-row').each(function () {
            ...
            if (svc === 'M') {
                ...
                $('#m_inventory_body .nalu-guide-e-detail-row').each(function () {
                    payload.Details.push({
            ...
            if (svc === 'D') {
                ...
                $('#d_shipment_body .nalu-guide-e-detail-row').each(function () {
            ...
            if (svc === 'C') {
                ...
                $('#c_cargo_body .nalu-guide-e-detail-row').each(function () {
                    var p = parseFloat($(this).find('.item-price').text()) || 0;
    ...
    function init(options) {
        ...
        bindDelivery();
        bindCarga();
        $('.nalu-guide-btn-cancel').on('click', function () { location.reload(); });
        bindSubmit();
